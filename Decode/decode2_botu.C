...
void
fake_transform_timestamp(unsigned int data, unsigned int &stamp)
{
  static int fake_n_frame = 0;

  unsigned int time_stamp = 0x10000*fake_n_frame*10 + (data & data_mask)*10;  // ns
  stamp = time_stamp;
...
//hist -----------------------
  ...
  ifstream fs("../Data/shot390825/ssm.dat",ios::binary | ios::in);//1M/spill
//  ifstream fs("../Data/shot391511/ssm.dat",ios::binary | ios::in);//20M/spill
  
//dataCont define
  std::vector<dataCont> data_list;
  std::vector<dataCont> fake_data_list;
  double prev_time = 0;
...
//Binary file scan  -----------------------
  ...
  fs.read((char*)&data, sizeof(unsigned int ));
        ...
        if(data == spill_end) continue;
                ...
                fake_data_list.push_back(tmp_data);
     ...
     // frame end ---------------------------
        ...
        std::vector<dataCont>::iterator fake_itr = fake_data_list.begin();
        // SORT!!! ---------------------------
        ...
        for(; fake_itr!=fake_data_list.end(); ++fake_itr){
                unsigned int fake_time;
                transform_timestamp((*fake_itr).data, fake_time);

                if(fake_time !=0 ){
                       double ms_fake_time = fake_time/ns_to_ms;
                       hist[0] -> Fill(ms_fake_time);
                }
        }//with claster sig
