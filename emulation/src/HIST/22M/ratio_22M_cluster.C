void ratio_22M_cluster(){
  TFile *file = new TFile("../../../rootfile/BH1_22M_cluster.root");
...
// TGraph --------------------------------------------------------------------
  ...
  TGraph *ch_ratio;
//  ch_ratio = new TGraph("ratio=max/ave","ratio = max / ave ",number_of_ch, ch,ratio);
...
// file.dat out --------------------------------------------------------------        
  TString fout1 = ("../DAT/maximum_22M_cluster.dat");
  ofstream fout_1(fout1.Data());
  TString fout2 = ("../DAT/average_22M_cluster.dat");
  ofstream fout_2(fout2.Data());
  TString fout3 = ("../DAT/ratio_22M_cluster.dat");
...
//      fout_3 << diff[i] << endl;
  ...
  TH1 *frame = gPad->DrawFrame(0,0,12,30,"Ratio");
         gPad->SetGrid();
         frame->GetXaxis()->SetTitle("ch");
         frame->GetYaxis()->SetTitleOffset(1.2);
         frame->GetYaxis()->SetTitle("Max [counts/10microsec] /Ave");
        ch_ratio->SetMarkerStyle(20);
        ch_ratio->SetMarkerColor(2);
        ch_ratio->Draw("p");
//  c2->Divide(3,1);
//        c2->cd(1);
//        ch_ratio->Draw();
...
//        ch_max->Draw();
 ...
 c1->Print("spillstructure_cluster.pdf");
 c2->Print("ratio_distribution_cluster.pdf");
