...
void dif(){
  TFile *file = new TFile("../../rootfile/BH1_1M.root");
...
// Calculation ---------------------------------------------------------------
   for(int i=0; i<300000; ++i){
        for(int j=0; j<12; ++j){
                count[j] = hist[j]->GetBinContent(i);
                if(count[j] > 3){
                        sum[j] = sum[j] + count[j];
                        ++number_of_bin[j];
                }
   ...
   for(int j=0; j<12; ++j){
        ...
        cout << "maximun:" << max_value[j] << " average:" << average_of_count[j] << " delta:" << diff[j] << endl;
   }
...
// TGraph --------------------------------------------------------------------
  ...
  TGraph *gr;
  gr = new TGraph(number_of_ch, ch,diff);
...
// file.dat out --------------------------------------------------------------        
 ...
 c2->Print("diff_distribution.pdf");
